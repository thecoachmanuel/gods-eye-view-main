...
async function geocodePlace(query, biasRect, signal) {
  ...
  if (apiKey) {
    ...
    try {
      ...
      // Fall through to geocode proxy fallback
    }
  }
  ...
  if (!result) {
    try {
      ...
      if (biasRect) proxyUrl += `&bias=${encodeURIComponent(biasRect)}`;
  ...
  const place = {
    ...
    primaryName: extractPrimaryName(result) || result.formatted_address?.split(',')?.[0]?.trim() || null,
    types: result.types || [],
